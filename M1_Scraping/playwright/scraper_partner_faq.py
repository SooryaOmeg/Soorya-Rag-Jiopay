...
def scrape_faq(url, save_dir="scraped_data"):
    ...
    with sync_playwright() as p:
        ...
        toggles = page.query_selector_all("div[tabindex='0']")
        print(f"Found {len(toggles)} Q&A toggles")

        for i, toggle in enumerate(toggles, start=1):
            try:
                # Extract question text
                q_el = toggle.query_selector("div.css-146c3p1.r-op4f77")
                question = q_el.inner_text().strip() if q_el else f"Q{i}"
                print(f"\n❓ Q{i}: {question}")

                # Locate chevron inside this toggle
                chevron = toggle.query_selector("div.css-146c3p1.r-kb43wt")
                if not chevron:
                    print("⚠️ No chevron found, skipping")
                    continue

                # Click chevron via JS (safer than .click())
                page.evaluate("(el) => el.click()", chevron)
                page.wait_for_timeout(500)

                # Look for sibling answer block
                ans_el = toggle.evaluate_handle(
                    """el => {
                        let wrapper = el.parentElement?.parentElement;
                        if (!wrapper) return "";
                        let ansBlock = wrapper.querySelector("div[data-testid='ViewTestId']");
                        return ansBlock ? ansBlock.innerText : "";
                    }"""
                )

                answer = ans_el.json_value().strip() if ans_el else ""
                if not answer:
                    print("⚠️ No answer extracted")
                else:
                    print(f"✅ A: {answer[:100]}...\n")
                if question != f"Q{i}": 
                    result["faq"].append({"question": question, "answer": answer})
    ...
    # Save JSON
    json_path = os.path.join(save_dir, "faq_combined.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
...
if __name__ == "__main__":
    url = "https://jiopay.com/business/partner-program"
    scrape_faq(url)
